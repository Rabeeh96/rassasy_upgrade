...
class POSPaymentController extends GetxController {
  ...
  Future<Null> getOrderDetails(
      {required String uID,
      required bool isData,
      required String responseData}) async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      try {
        ...
        if (status == 6000) {
          ...
          exciseAmountTotalP.value =
              double.parse(responseJson["ExciseTaxAmount"].toString());
          totalGrossP.value =
  ...
  calculationOnPayment() {
    ...
    var gt = (net - disCount.value).toString();
    grandTotalAmount.value = roundStringWith(gt).toString();
    billDiscPercent.value =
        (disCount * 100 / double.parse(grandTotalAmount.value));
    balance.value =
        (net - disCount.value) - (cashReceived.value + bankReceived.value);
    update();
  }

  final POSController posController = Get.put(POSController());
  Future<Null> createSaleInvoice(
      {required bool printSave,
    ...
    if (connectivityResult == ConnectivityResult.none) {
      ...
      try {
        print(
            "---------------printSave--------------$printSave----------tableID--------$tableID------------uUID--------$uUID-------orderType---------$orderType-------------------------");
        start(context);
