...
@override
  ...
  Future<Null> posFunctions({required String sectionType,required String uUID}) async {
   ...
   // productList.clear();
    groupList.clear();
      ...
      print("----quantityIncrement----------quantityIncrement---------$quantityIncrement");
  ...
  Future<void> getProductListDetail(groupID) async {
    try {
      ...
      print(url);
       Map data = {"CompanyID": companyID, "BranchID": branchID, "GroupID": groupID, "type": isVegNotifier.value ?'veg':"", "PriceRounding":priceRounding};
      print(data);
      //encode Map to JSON
      var body = json.encode(data);
...
//   }
  ...
  /// function for create
  Future<Null> createSalesOrderRequest({ required BuildContext context,required bool isPayment,required int orderType,required String  tableID,required String tableHead}) async {
    start(context);
    try {
