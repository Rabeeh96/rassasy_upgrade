...
class ThermalArabicShort {

  /// wifi
  static String invoiceDesign(width) {
    ...
    var countyCodeCompany = BluetoothPrintThermalDetails.currency;

    print('-------------country code $countyCodeCompany');
    if(countyCodeCompany =="SR"){
      companyTax = BluetoothPrintThermalDetails.vatNumberCompany;
    }
    else if(countyCodeCompany == "Rs"){
      companyTax = BluetoothPrintThermalDetails.companyGstNumber;
    }
    ...
    var totalQty = BluetoothPrintThermalDetails.totalQty;

    var voucherNumber = BluetoothPrintThermalDetails.voucherNumber;
    var name = BluetoothPrintThermalDetails.customerName;
    var customerVatNumber = BluetoothPrintThermalDetails.customerTaxNumber;
    var customerCrNumber = BluetoothPrintThermalDetails.customerCrNumber;
    ...
    var qrCode = BluetoothPrintThermalDetails.qrCodeImage;


    var qr = """ <center> <div class="line-1"></div>

    <div><img class='qrCodeImage' src='$qrCode'/></div>
    </center>""";
    if (BluetoothPrintThermalDetails.invoiceType == "SR") {
      invoiceType = "CREDIT NOTE";
      invoiceTypeArabic = "(اشعار دائن)";
      qr = """ <center> <div class="line-1"></div>
    ...
    var printHead = """<table style="margin-bottom:unset">
        ...
        <th class="textCenter">SL<br><font size="5">(رقم)</font></th>  -->
               ...
               <th class="textRight" style=" padding:0px 15px">Unit<br><font size="5">(مجموع)</font></th>
    ...
    for (var i = 0; i < tableDataDetailsPrint.length; i++) {
      ...
      printItems += """<tr>
               ...
               <td style="width: 65%;  padding:0px 15px"><span style="font-weight: 600;font-size: 24px;">${tableDataDetailsPrint[i].productDescription}</span></td>
               <td class="textRight" style="width: 17% ;  padding:0px 15px;font-size: 22px;" >${roundStringWith(tableDataDetailsPrint[i].qty)}</td>
               <td class="textRight" style='font-weight: 800;  padding:0px 15px;font-size: 22px;'>${roundStringWith(tableDataDetailsPrint[i].unitPrice)}</td>
               <td class="textRight" style='font-weight: 800;  padding:0px 15px;font-size: 22px;'>${roundStringWith(tableDataDetailsPrint[i].netAmount)}</td>
...
width: 60%
  ...
  <tr>
    <td colspan="2" class="bold">$companyName</td>
  </tr>
  <tr>
    <td colspan="2" class="bold">$companyAddress1</td>
  </tr>
     <tr>
    <td colspan="2" class="bold">$companyCountry</td>
  </tr>
 
    
    <td colspan="2" class="bold"> الرقم الضريبي : $companyTax  </td>
  </tr>
 
  <tr>
    <td colspan="2"  class="bold">$companyCrNumber</td>
  </tr>
  <tr>
    <td colspan="2"   class="bold">$companyPhone</td>
  </tr>
   ...
   <tr>
    <td colspan="2">$invoiceType</td>
       ...
       <div class="child parent-column">
    	<div> </div>
    	<div>Inv No(قم الفاتورة) : $voucherNumber</div>
        <div>Date(تاريخ) : $date</div>
    	<div> </div>
    </div>
	<div class="child parent-column">
         <div>Name(اسم) : $name</div>
    	<div>cr(س. ت) : $customerCrNumber</div>
        <div>vat No(ر ض) :$customerVatNumber</div>
      ...
      $printItems
         ...
         <table style="width:100%;border: none margin-bottom:unset">
              ...
              <tr>
                <td class="b-right bold">Grand total/المبلغ الإجمالي</td>
                <td class="b-right w-25 bold">$countyCodeCompany $grandTotal</td>
              </tr>
