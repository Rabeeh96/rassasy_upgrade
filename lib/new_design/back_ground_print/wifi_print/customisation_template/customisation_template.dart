...
class WifiPrintClassTest {
  ...
  Future<Uint8List> _fetchImageData(l) async {
    final http.Response response = await http.get(Uri.parse(l));
  ...
  printReq(arabicImageBytes,qrCode, needQR,voucherType,printerIpAddress,companyLogo,countyCodeCompany,numberOfCopies) async {
    ...
    final PosPrintResult res = await printer.connect(printerIpAddress, port: port,);
    if (res == PosPrintResult.success) {
      ...
      for(var i = 0;i<numberOfCopies ;i++){
        ...
        print("-------------------------------------------------------22");
        await printDemoPage(printer, paper,arabicImageBytes,qrCode, needQR,voucherType,companyLogo,countyCodeCompany);
        print("-------------------------------------------------------33");
      }
      print("-------------------------------------------------------44");
      print("===DateTime===${DateTime.now().second}");
      Future.delayed( Duration(seconds: numberOfCopies+2), () async {
       print("===DateTime===  before disconnect${DateTime.now().second}");
        print("-------------------------------------------------------55");

        printer.disconnect();
       print("===DateTime=== after disconnect  ${DateTime.now().second}");
        print("-------------------------------------------------------66");
      });
    }

  }
  Future<void> printDemoPage(NetworkPrinter printer,PaperSize paper,arabicImageBytes,qrCode, needQR,type,companyLogo,countyCodeCompany) async {
    if(countyCodeCompany =="QAR"){
      needQR = false;
    }
    if (type == "SI") {
        if (companyLogo != "") {
  ...
  Future<Uint8List> generateInvoice(
    ...
    if(isCancelled){
      ...
      final cancelNoteBuilder =
      ui.ParagraphBuilder(companyDetailsParagraphStyle)
        ..pushStyle(ui.TextStyle(color: Colors.black, fontSize: 30.0))
        ..addText("CUSTOMER CANCELLED THE ORDER");
      final cancelNoteBuilderParagraph = cancelNoteBuilder.build()
        ..layout(ui.ParagraphConstraints(width: canvasSize1.width));
      canvas.drawParagraph(
          cancelNoteBuilderParagraph, Offset(0, positionHeight));
...
// Layout constraints for each column
  ...
  Future<void> kotPrintConnect(
    ...
    try {
      ...
      if (res == PosPrintResult.success) {
        print("Printer connection Success -------------------------");
        final arabicImageBytes = await generateKOT(
            id: id,
            items: items,
            isCancelNote: isCancelNote,
            isUpdate: isUpdate);
        await printKOT(printer, paper,arabicImageBytes);
  ...
  Future<void> printKOT(NetworkPrinter printer,PaperSize paper,arabicImageBytes) async {
